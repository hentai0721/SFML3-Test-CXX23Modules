ifeq ($(shell file $$(which make) | awk '/PE32/ {found=1; exit} END {if (found) print "PE32"}'),PE32)
CXX = clang++
...
CXX = clang++
CXXFLAGS = -stdlib=libc++ -O3 -std=c++23 -fprebuilt-module-path=$(RELEASE_PATH) --precompile
CXXFLAGS_P = -O3 -std=c++23 -fprebuilt-module-path=$(RELEASE_PATH)
LDFLAGS = -Wl,-s
